...
### 4.1 API Migration (REST → GraphQL)
...
### 4.3 Large-Scale Refactor (Class → Hooks)

**Problem:** 50 React class components → functional + hooks
...
for file in $COMPONENTS; do
  ...
  if [ $? -eq 0 ]; then
    ...
    echo "✗ $file failed tests - manual intervention needed"
    break
  fi
done
...
**Problem:** React 17 → 18 (breaking changes: concurrent rendering, new APIs)
...
- Reduces ambiguity ("make it look good" → exact visual reference)
...
Make changes:
1. Title → text-5xl font-bold
2. Button → larger (h-14 px-8), vibrant color
3. Background → darker gradient for contrast
...
**Pattern:** Find UI you like → recreate it
...
- [ ] Break migrations into phases (analysis → plan → execute)
